...
.loop
	ld a, [hMultiplier]
	srl a
	ld [hMultiplier], a ; (aliases: hDivisor, H_MULTIPLIER, hPowerOfTen)
	...
	ld c, a
	ld a, [hMultiplicand] ; (aliases: H_MULTIPLICAND)
	adc c
	ld [hMultiplyBuffer+1], a
	ld a, [hMultiplyBuffer]
	ld c, a
	ld a, [hProduct] ; (aliases: H_PRODUCT, hPastLeadingZeroes, hQuotient)
	adc c
	ld [hMultiplyBuffer], a
.smallMultiplier
	dec b
...
.asm_37db3
	...
	sub c
	ld d, a
	ld a, [hDivisor] ; (aliases: H_DIVISOR, hMultiplier, hPowerOfTen)
	ld c, a
	ld a, [hDividend] ; (aliases: hProduct, hPastLeadingZeroes, hQuotient)
	sbc c
	jr c, .asm_37dce
	ld [hDividend], a ; (aliases: hProduct, hPastLeadingZeroes, hQuotient)
	ld a, d
...
.asm_37dce
	...
	ld e, a
	ld a, [hDivideBuffer]
	ld [hDivisor], a ; (aliases: H_DIVISOR, hMultiplier, hPowerOfTen)
	xor a
	ld [hDivideBuffer], a
	ld a, [hDividend+1] ; (aliases: hMultiplicand)
	ld [hDividend], a ; (aliases: hProduct, hPastLeadingZeroes, hQuotient)
...
.asm_37e18
	ld a, [hDividend+1] ; (aliases: hMultiplicand)
	ld [hRemainder], a ; (aliases: hDivisor, hMultiplier, hPowerOfTen)
	ld a, [hDivideBuffer+4]
	ld [hQuotient+3], a
	ld a, [hDivideBuffer+3]
	ld [hQuotient+2], a
